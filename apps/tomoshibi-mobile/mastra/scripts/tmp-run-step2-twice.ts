...
const pick = (x: Awaited<ReturnType<typeof generateSeriesConcept>>) => ({
  title: x.title,
  genre: x.genre,
  tone: x.tone,
  case_core: x.mystery_profile.case_core,
  investigation_style: x.mystery_profile.investigation_style,
  truth_nature: x.mystery_profile.truth_nature,
  duo_dynamic: x.mystery_profile.duo_dynamic,
  visual_language: x.mystery_profile.visual_language,
  environment_layer: x.mystery_profile.environment_layer,
});

const main = async () => {
  console.log("RUN#1_START");
  const r1 = await generateSeriesConcept(input);
  console.log("RUN#1_RESULT");
  console.log(JSON.stringify(pick(r1), null, 2));

  console.log("RUN#2_START");
  const r2 = await generateSeriesConcept(input);
  console.log("RUN#2_RESULT");
  console.log(JSON.stringify(pick(r2), null, 2));

  const sameCase = r1.mystery_profile.case_core === r2.mystery_profile.case_core;
  const sameTitle = r1.title === r2.title;
  console.log("COMPARE");
  console.log(JSON.stringify({ sameTitle, sameCase }, null, 2));
};

main().catch((e) => {
  console.error("STEP2_TWICE_FAILED");
  console.error(e);
  process.exitCode = 1;
});
