...
import type { Character, CharacterAppearance, CharacterPart } from "../src/types/character";
import type { AreaMode, PlaceAnnotation } from "../src/types/place";

if (!process.env.FIRESTORE_EMULATOR_HOST) {
  throw new Error("FIRESTORE_EMULATOR_HOST is required. Run with npm run seed:local while Firestore Emulator is running.");
}

if (getApps().length === 0) {
  initializeApp({
    projectId: "tomoshibi-local",
  });
}
...
async function main(): Promise<void> {
  ...
  await Promise.all(annotations.map((annotation) => db.collection("placeAnnotations").doc(annotation.id).set(annotation)));

  console.log(`Seeded characters=${characters.length}, appearances=${appearances.length}, parts=${characterParts.length}, areaMode=${areaMode.id}, annotations=${annotations.length}`);
}
