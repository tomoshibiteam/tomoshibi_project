...
function shouldUseFirebaseEmulators(): boolean {
  ...
  // 実機アクセス時は loopback emulator を無効化して Cloud Functions を使う。
  const emulatorHost = readTrimmedEnv(process.env.NEXT_PUBLIC_FIREBASE_FUNCTIONS_EMULATOR_HOST);
  return Boolean(emulatorHost && !isLoopbackHost(emulatorHost));
}
