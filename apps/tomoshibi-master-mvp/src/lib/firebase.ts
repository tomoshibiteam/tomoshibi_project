...
function shouldUseFirebaseEmulators(): boolean {
  ...
  // Build/SSR 側では従来挙動を維持し、ブラウザ実行時のみ接続先の安全判定を行う。
  ...
  // 実機アクセス時は、emulator host が loopback なら到達不能なので Cloud 側へフォールバックする。
  const firestoreHost = readOptionalEnv(process.env.NEXT_PUBLIC_FIREBASE_FIRESTORE_EMULATOR_HOST);
  const functionsHost = readOptionalEnv(process.env.NEXT_PUBLIC_FIREBASE_FUNCTIONS_EMULATOR_HOST);
  const emulatorHosts = [firestoreHost, functionsHost].filter((host): host is string => Boolean(host));
  return emulatorHosts.some((host) => !isLoopbackHost(host));
}

function printRuntimeLog(mode: "cloud" | "emulator") {
  if (runtimeLogPrinted || typeof window === "undefined") return;
  runtimeLogPrinted = true;

  console.info("[Firebase] runtime", {
    mode,
    host: window.location.hostname,
    projectId: process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID ?? null,
    firestoreEmulatorHost: process.env.NEXT_PUBLIC_FIREBASE_FIRESTORE_EMULATOR_HOST ?? null,
    firestoreEmulatorPort: process.env.NEXT_PUBLIC_FIREBASE_FIRESTORE_EMULATOR_PORT ?? null,
    functionsEmulatorHost: process.env.NEXT_PUBLIC_FIREBASE_FUNCTIONS_EMULATOR_HOST ?? null,
    functionsEmulatorPort: process.env.NEXT_PUBLIC_FIREBASE_FUNCTIONS_EMULATOR_PORT ?? null,
  });
}
...
export function getFirebaseClientFirestore(): Firestore | null {
  ...
  if (!firestoreInstance) {
    if (typeof window === "undefined") {
      ...
      // Mobile / proxy 経由の WebChannel 不達で unavailable が出るケースを吸収する。
      try {
