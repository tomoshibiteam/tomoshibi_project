...
function buildFallbackNarrative(params: {
  ...
  routeIds: string[];
  tone: string;
}): NarrativeStep6Result {
  const routeSpots = params.routeIds.map((id) => SPOT_BY_ID[id]).filter((spot): spot is SpotRecord => Boolean(spot));
  const routeNames = routeSpots.map((spot) => spot.name).join(" → ");
  const roleTemplates = resolveSpotRoleTemplates(routeSpots.length);

  return {
    generatedStoryName: `${params.userType}の伊都キャンパスクエスト`,
    storyTone: params.tone,
    readyHeroLead: `${params.duration}で巡る伊都キャンパスルート`,
    readySummaryTitle: `${params.userType}向けの体験準備が完了しました`,
    readySummaryText: `${params.familiarity}の来訪者向けに、${routeNames}をつなぐ導線を構成しました。探索スタイル「${params.explorationStyle}」と期待「${params.expectation}」に沿って、無理なく巡れる順序に整えています。`,
    prologueBody: KYUDAI_ISHIGAHARA_PROLOGUE_BODY,
    epilogueBody: KYUDAI_ISHIGAHARA_EPILOGUE_BODY,
    spots: routeSpots.map((spot, index) => ({
      id: spot.id,
      name: spot.name,
      overview: spot.overview,
      rationale:
        roleTemplates[index] ??
        (index === routeSpots.length - 1 ? "終着点で全体をつなぐ" : "ルートの連続性を維持するため"),
      scenarioTexts: [
        `まもなく${spot.name}に到着します。${roleTemplates[index] ?? "この地点の役割"}を意識して進みましょう。`,
        `${spot.name}に到着しました。${spot.facts[0] ?? spot.overview}`,
        index < routeSpots.length - 1
          ? `${spot.facts[1] ?? "この地点の使われ方"}を踏まえて、「${params.expectation}」につながる視点を持ったまま次の${routeSpots[index + 1]?.name ?? "地点"}へ向かいましょう。`
          : `ここまでのスポットを一本の行動導線として結び直し、次に何をしに来るかを言語化して締めましょう。`,
      ],
    })),
  };
}
...
function sanitizeNarrativeResult(input: NarrativeStep6Result, routeIds: string[]): NarrativeStep6Result {
  ...
  const sanitizedSpots = routeIds.map((id) => {
    ...
    const scenarioTexts = toStringArray(source?.scenarioTexts).slice(0, 3);
    const fallbackScenarioTexts = [
      `まもなく${fallbackSpot.name}に到着します。`,
      `${fallbackSpot.name}に到着しました。${fallbackSpot.facts[0] ?? fallbackSpot.overview}`,
      `${fallbackSpot.facts[1] ?? "この地点の特徴"}を押さえ、次の地点へ接続しましょう。`,
    ];

    return {
      id,
      name: toText(source?.name, fallbackSpot.name),
      overview: toText(source?.overview, fallbackSpot.overview),
      rationale: toText(source?.rationale, "ルートの連続性と学習効果を優先して選定。"),
      scenarioTexts: scenarioTexts.length === 3 ? scenarioTexts : fallbackScenarioTexts,
    };
  });

  return {
    generatedStoryName: toText(input.generatedStoryName, "伊都キャンパスクエスト"),
    storyTone: toText(input.storyTone, "落ち着き・知的・親しみやすい"),
    readyHeroLead: toText(input.readyHeroLead, "伊都キャンパス探索ルート"),
    readySummaryTitle: toText(input.readySummaryTitle, "体験の準備が整いました"),
    readySummaryText: toText(input.readySummaryText, "ルート条件に沿ったクエストを作成しました。"),
    prologueBody: toText(input.prologueBody, KYUDAI_ISHIGAHARA_PROLOGUE_BODY),
    epilogueBody: toText(input.epilogueBody, KYUDAI_ISHIGAHARA_EPILOGUE_BODY),
...
function validateNarrativeResult(input: NarrativeStep6Result, routeIds: string[]): string[] {
  ...
  for (const id of routeIds) {
    ...
    if (scenarioTexts.length < 3) {
      errors.push(`${id} の scenarioTexts は3要素必要です`);
    }
  }
...
async function callOpenAiJson<T>(params: {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        data: null,
        error: `OpenAI API エラー: ${response.status} ${detail}`,
      };
    }
    ...
    if (typeof content !== "string" || content.trim().length === 0) {
      return {
        data: null,
        error: "OpenAIレスポンスの content が空です。",
      };
    }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const maybeReturnPartial = (stepId: GenerationStepId): NextResponse | null => {
    ...
    log("step6.ai.request", {
      providerCandidates: ["openai", "gemini"],
    ...
    const step6OpenAi = await callOpenAiJson<NarrativeStep6Result>({
      systemPrompt: step6SystemPrompt,
      userPrompt: step6UserPrompt,
      temperature: 0.45,
    });
    const step6Gemini = step6OpenAi.data
      ? null
      : await callGeminiJson<NarrativeStep6Result>({
          systemPrompt: step6SystemPrompt,
          userPrompt: step6UserPrompt,
          temperature: 0.45,
    ...
    let step6UsedProvider: "openai" | "gemini" | null = null;
    let step6UsedModel: string | null = null;

    if (step6OpenAi.data || step6Gemini?.data) {
      narrativeResult = (step6OpenAi.data ?? step6Gemini?.data) as NarrativeStep6Result;
      step6UsedProvider = step6OpenAi.data ? "openai" : "gemini";
      step6UsedModel = step6OpenAi.data ? OPENAI_MODEL : (step6Gemini?.model ?? GEMINI_NARRATIVE_MODEL);

      log("step6.ai.response", {
        usedFallback: false,
        provider: step6UsedProvider,
        model: step6UsedModel,
        openAiError: step6OpenAi.error,
        geminiError: step6Gemini?.error ?? null,
      ...
      stepEnd(
        {
          ...
          label: "差し込み生成",
          status: "completed",
          detail: `AIで ready/prologue/spot/epilogue を生成しました (${step6UsedProvider})`,
        },
        {
          provider: step6UsedProvider,
          model: step6UsedModel,
      ...
      stepTrace({
        id: "step6",
        program:
          "step6InsertionGenerationプロンプトへworld/character/arc設定とspot-db知識候補を埋め込み、route順spot本文をAI生成する（openai失敗時はgeminiへフォールバック）。",
        inputVars: {
          ...
          routeSpotKnowledge,
          providerErrors: {
            openai: step6OpenAi.error,
            gemini: step6Gemini?.error ?? null,
          },
        },
        outputVars: {
          status: "completed",
        ...
        aiPrompt: {
          provider: step6UsedProvider,
          model: step6UsedModel ?? (step6UsedProvider === "openai" ? OPENAI_MODEL : GEMINI_NARRATIVE_MODEL),
          temperature: 0.45,
          systemPrompt: step6SystemPrompt,
          userPrompt: step6UserPrompt,
        },
      ...
      narrativeResult = buildFallbackNarrative({
        userType: normalizedUserType,
        familiarity: normalizedFamiliarity,
        duration: normalizedDuration,
        explorationStyle,
      ...
      logError("step6.ai.response_error", {
        usedFallback: true,
        openAiError: step6OpenAi.error ?? "unknown",
        geminiError: step6Gemini?.error ?? "not_attempted_or_unknown",
        fallbackPayload: narrativeResult,
      });
      stepEnd(
        {
          id: "step6",
          label: "差し込み生成",
          status: "fallback",
          detail: `fallback narrative を使用 (openai=${step6OpenAi.error ?? "unknown"} / gemini=${step6Gemini?.error ?? "not_attempted"})`,
        },
        {
          generatedStoryName: toText(narrativeResult.generatedStoryName),
          generatedSpotsCount: Array.isArray(narrativeResult.spots) ? narrativeResult.spots.length : 0,
      ...
      stepTrace({
        id: "step6",
        program:
          "openai/gemini の両方で生成に失敗した場合はfallback narrative builderで本文を生成する（spot-db知識差し込みは未適用）。",
        inputVars: {
          promptTemplate: step6PromptTemplate,
          promptResolved: step6Prompt,
          routeIds,
    ...
    if (validationErrors.length > 0) {
      ...
      const step7OpenAi = await callOpenAiJson<NarrativeStep6Result>({
        systemPrompt: step7SystemPrompt,
        userPrompt: step7UserPrompt,
        temperature: 0.1,
      });
      const step7Gemini = step7OpenAi.data
        ? null
        : await callGeminiJson<NarrativeStep6Result>({
            systemPrompt: step7SystemPrompt,
            userPrompt: step7UserPrompt,
      ...
      const step7UsedProvider: "openai" | "gemini" | null = step7OpenAi.data
        ? "openai"
        : step7Gemini?.data
          ? "gemini"
          : null;
      const step7UsedModel =
        step7UsedProvider === "openai"
          ? OPENAI_MODEL
          : step7UsedProvider === "gemini"
            ? (step7Gemini?.model ?? GEMINI_NARRATIVE_MODEL)
            : null;
      const step7Result = (step7OpenAi.data ?? step7Gemini?.data) as NarrativeStep6Result | null;

      if (step7Result) {
        narrativeResult = step7Result;
        log("step7.ai.response_payload", {
          provider: step7UsedProvider,
          model: step7UsedModel,
          openAiError: step7OpenAi.error,
          geminiError: step7Gemini?.error ?? null,
          payload: step7Result,
        });
        ...
        if (afterRepairErrors.length === 0) {
          ...
          stepEnd({
            ...
            label: "検証/最小再生成",
            status: "completed",
            detail: `AIで最小修正を実行し、検証を通過しました (${step7UsedProvider})`,
          });
          stepTrace({
            id: "step7",
            program: "検証失敗時にstep7MinimalRepairプロンプトで最小再生成し、再検証で通過を確認する（openai失敗時はgeminiへフォールバック）。",
            inputVars: {
              validationErrors,
              promptTemplate: step7PromptTemplate,
              promptResolved: step7Prompt,
              providerErrors: {
                openai: step7OpenAi.error,
                gemini: step7Gemini?.error ?? null,
              },
            },
            outputVars: {
              status: "completed",
            ...
            aiPrompt: {
              provider: step7UsedProvider ?? "openai",
              model: step7UsedModel ?? OPENAI_MODEL,
              temperature: 0.1,
              systemPrompt: step7SystemPrompt,
              userPrompt: step7UserPrompt,
            },
          ...
          stepEnd({
            ...
            label: "検証/最小再生成",
            status: "fallback",
            detail: `AI修正後も検証失敗: ${afterRepairErrors.join(" / ")}。サニタイズを適用します。`,
          });
          stepTrace({
            id: "step7",
            program:
              "最小再生成後も検証不通過の場合はfallback扱いで後段のsanitize処理へ進む。",
            inputVars: {
              validationErrors,
              promptTemplate: step7PromptTemplate,
              promptResolved: step7Prompt,
        ...
        logError("step7.ai.response_error", {
          usedFallback: true,
          openAiError: step7OpenAi.error ?? "unknown",
          geminiError: step7Gemini?.error ?? "not_attempted_or_unknown",
        });
        stepEnd({
          id: "step7",
          label: "検証/最小再生成",
          status: "fallback",
          detail: `AI修正を実行できませんでした (openai=${step7OpenAi.error ?? "unknown"} / gemini=${step7Gemini?.error ?? "not_attempted"})。サニタイズを適用します。`,
        });
        stepTrace({
          id: "step7",
          program: "検証失敗時にAI修正呼び出しが失敗した場合はfallbackでsanitizeへ進む。",
          inputVars: {
            validationErrors,
            promptTemplate: step7PromptTemplate,
            promptResolved: step7Prompt,
            providerErrors: {
              openai: step7OpenAi.error ?? null,
              gemini: step7Gemini?.error ?? null,
            },
          },
          outputVars: {
            status: "fallback",
            error: `openai=${step7OpenAi.error ?? "unknown"} / gemini=${step7Gemini?.error ?? "unknown"}`,
          },
        });
      }
    } else {
    ...
    const finalQuest: QuestOutput = {
      generatedStoryName: toText(sanitizedNarrative.generatedStoryName, "伊都キャンパスクエスト"),
      storyTone: toText(sanitizedNarrative.storyTone, tone),
      readyHeroLead: toText(sanitizedNarrative.readyHeroLead, "伊都キャンパス探索ルート"),
      readySummaryTitle: toText(sanitizedNarrative.readySummaryTitle, "体験の準備が整いました"),
      readySummaryText: toText(sanitizedNarrative.readySummaryText, "ルートを生成しました。"),
      prologueBody: toText(sanitizedNarrative.prologueBody, KYUDAI_ISHIGAHARA_PROLOGUE_BODY),
      epilogueBody: toText(sanitizedNarrative.epilogueBody, KYUDAI_ISHIGAHARA_EPILOGUE_BODY),
      spots: routeSpots.map((spot) => {
        const generated = narrativeSpotMap.get(spot.id);
        return {
          id: spot.id,
          name: toText(generated?.name, spot.name),
          overview: toText(generated?.overview, spot.overview),
          rationale: toText(generated?.rationale, routeReasonMap[spot.id] ?? "ルート全体の連続性を優先して選定。"),
          scenarioTexts: toStringArray(generated?.scenarioTexts).slice(0, 3),
          lat: spot.lat,
          lng: spot.lng,
        };
      }),
