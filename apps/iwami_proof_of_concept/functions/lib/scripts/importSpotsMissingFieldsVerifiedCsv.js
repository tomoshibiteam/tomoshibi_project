...
const UNRESOLVED_KEYWORDS = /未確認|未公開|要確認|公開なし|明記なし|要問合せ|不定休|変則|通行止め|随時対応|食い違う|休業あり|混雑時に変更/;
...
function parseWeeklyHoursRaw(value) {
    if (!value)
        return {};
    try {
...
function parseWeeklyHoursStructured(raw) {
    const out = {};
    for (const day of ["mon", "tue", "wed", "thu", "fri", "sat", "sun"]) {
        const text = raw[day];
        if (!text)
...
function parseRegularClosedDays(value) {
    if (!value)
        return [];
    ...
    for (const segment of segments) {
        if (!/(休|closed|定休|休館|休業)/i.test(segment))
            continue;
        if (/除く/.test(segment))
            continue;
        if (/平日/.test(segment)) {
        ...
        const shorthandClosed = segment.matchAll(/(?:^|[・\/、,\s])([月火水木金土日](?:[・\/、,\s]+[月火水木金土日])*)休/g);
    ...
    const ordered = ["mon", "tue", "wed", "thu", "fri", "sat", "sun", "holiday"];
    return ordered.filter((day) => days.has(day));
}
function parseSeasonalClosures(value) {
    if (!value)
        return [];
    const text = value.trim();
    if (!text)
        return [];
    if (/^(なし|特になし|記載なし)$/i.test(text))
        return [];
    const closures = [];
    const seen = new Set();
    const dateRanges = text.matchAll(/(\d{4})[\/.-](\d{1,2})[\/.-](\d{1,2})\s*[-〜～]\s*(\d{4})[\/.-](\d{1,2})[\/.-](\d{1,2})/g);
...
function parseLastAdmission(value) {
    if (!value) {
    ...
    const markerMatch = text.match(/(?:L\.?\s*O\.?|ラストオーダー|最終受付|最終入場|最終便|受付締切|予約締切)[^0-9]{0,12}([0-2]?\d[:：][0-5]\d)/i);
...
async function main() {
    ...
    const csvText = await (0, promises_1.readFile)(csvPath, "utf8");
    const workbook = (0, xlsx_1.read)(csvText, { type: "string", raw: false });
    ...
    const rows = xlsx_1.utils.sheet_to_json(sheet, { defval: "" });
    let updatedCount = 0;
    let skippedCount = 0;
    const skipped = [];
    for (const [index, row] of rows.entries()) {
        ...
        const hasIrregularClosures = hasIrregularExplicit ??
            /不定休|臨時|変則|通行止め|随時|要確認/.test([regularClosedText, seasonalText, notes].filter((item) => Boolean(item)).join(" / "));
        const needsManualReview = UNRESOLVED_KEYWORDS.test([lastAdmissionText, regularClosedText, seasonalText, notes]
            .filter((item) => Boolean(item))
            .join(" / ")) || confidence?.startsWith("low") === true;
    ...
    console.log(`[importSpotsMissingFieldsVerifiedCsv] completed file=${csvPath} totalRows=${rows.length} updated=${updatedCount} skipped=${skippedCount}`);
    if (skipped.length > 0) {
...
//# sourceMappingURL=importSpotsMissingFieldsVerifiedCsv.js.map