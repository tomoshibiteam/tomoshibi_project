...
exports.computeApproximateRouteMatrix = computeApproximateRouteMatrix;
const APPROXIMATION_PROFILE_BY_MODE = {
    walk: {
...
function computeApproximateMinutes(distanceMeters, mode) {
    if (distanceMeters <= 0)
        return 0;
...
async function computeApproximateRouteMatrix(params) {
    ...
    for (const from of params.locations) {
        for (const to of params.locations) {
            ...
            const distance = Math.max(0, Math.round(haversineMeters({ lat: from.lat, lng: from.lng }, { lat: to.lat, lng: to.lng })));
...
//# sourceMappingURL=approximateRouteMatrix.js.map