...
exports.applyGtfsOverlays = applyGtfsOverlays;
...
function includesAny(targets, keywords) {
...
function inferRouteVariant(params) {
    ...
    if (hasTago && hasKugamiSide)
        return "tago_to_kugami";
    if (hasTago && !hasKugamiSide)
        return "tago_terminating";
    if (!hasTago && hasKugamiSide)
        return "kugami_direct";
    return null;
}
function isShoppingTrip(stopNames) {
        ...
        if (service.serviceId === "bus_iwami_tago_kugami_line") {
            return schemas_1.transitServiceRecordSchema.parse({
                ...
                metadata: {
                    ...
                    shoppingStops: Array.from(new Set([...(service.metadata?.shoppingStops ?? []), "サンマート岩美店", "道の駅きなんせ岩美"])),
                    remarks: Array.from(new Set([...(service.metadata?.remarks ?? []), "買い物便あり", "令和8年4月1日改正"])),
                    routeVariantHints: Array.from(new Set([...(service.metadata?.routeVariantHints ?? []), "tago_terminating", "tago_to_kugami", "kugami_direct"])),
                },
            });
        }
        return service;
