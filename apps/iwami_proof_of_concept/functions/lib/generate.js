...
exports.generatePlans = generatePlans;
exports.runPlanGenerationWithFailureHandling = runPlanGenerationWithFailureHandling;
...
const approximateRouteMatrix_1 = require("./routing/approximateRouteMatrix");
const repository_2 = require("./transit/repository");
...
const themeWeightsSchema = zod_1.z.object({
    scenic: zod_1.z.number().min(0).max(5),
    food: zod_1.z.number().min(0).max(5),
    experience: zod_1.z.number().min(0).max(5),
    shopping: zod_1.z.number().min(0).max(5),
    stationArea: zod_1.z.number().min(0).max(5),
});
const intentSchema = zod_1.z.object({
    must: zod_1.z.array(zod_1.z.string()),
    prefer: zod_1.z.array(zod_1.z.string()),
    avoid: zod_1.z.array(zod_1.z.string()),
    pace: zod_1.z.enum(["relaxed", "normal", "active"]),
...
const refinementBatchSchema = zod_1.z.object({
    plans: zod_1.z.array(zod_1.z.object({
        id: zod_1.z.string().min(1),
        title: zod_1.z.string().min(1),
        reasonWhyRecommended: zod_1.z.string().min(1),
        matchSummary: zod_1.z.string().min(1),
    })),
});
...
function getCurrentDayType(now = new Date()) {
...
async function callGeminiJson(params) {
    ...
    if (!apiKey) {
        throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.AI_PARSE_FAILED, "Gemini API key is not configured", {
    ...
    if (!model) {
        throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.AI_PARSE_FAILED, "Gemini model is not configured", {
            context: params.context,
            required: ["PLAN_GEMINI_MODEL", "GEMINI_ROUTE_MODEL"],
        });
    }
    const endpoint = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(apiKey)}`;
    const backoff = (attempt) => Math.min(8_000, 450 * Math.pow(2, attempt - 1) + Math.round(Math.random() * 350));
    for (let attempt = 1; attempt <= GEMINI_MAX_ATTEMPTS; attempt += 1) {
        ...
        try {
            ...
            throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.AI_PARSE_FAILED, "Gemini call failed", {
                context: params.context,
                reason: timeoutLike ? "timeout" : "network_error",
        ...
        if (!response.ok) {
            ...
            throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.AI_PARSE_FAILED, "Gemini response not ok", {
                context: params.context,
        ...
        try {
            ...
            throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.AI_PARSE_FAILED, "Gemini response JSON parse failed", {
                context: params.context,
                detail: error instanceof Error ? error.message : String(error),
            });
        }
        ...
        if (!text) {
            ...
            throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.AI_PARSE_FAILED, "Gemini returned empty JSON text", {
                context: params.context,
                model,
            });
        }
        try {
            ...
            throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.AI_PARSE_FAILED, "Gemini schema parse failed", {
                context: params.context,
    ...
    throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.AI_PARSE_FAILED, "Gemini retry limit exceeded", {
        context: params.context,
...
async function extractIntent(request) {
    return callGeminiJson({
        systemInstruction: "あなたは旅行プランナーです。与えられた条件から意図を構造化JSONで返してください。出力はJSONのみ。",
        userPrompt: [
...
function includesAnyText(base, needles) {
    if (needles.length === 0)
        return false;
...
function resolvePrimaryTransport(localTransports) {
    const priority = ["car", "rental_cycle", "bus", "walk"];
    for (const transport of priority) {
        if (localTransports.includes(transport))
            return transport;
    }
    return "walk";
}
function toRouteMatrixMode(mode) {
...
function isTransportCompatible(spot, localTransports) {
    return localTransports.some((transport) => spot.access.supportedTransports.includes(transport));
}
function scoreSpot(spot, style, intent, request) {
    ...
    return (planner.priorityScore * 0.35 +
        styleScore +
        desiredHit +
        mustHit +
        preferHit +
...
function selectCandidates(spots, intent, request) {
    ...
    return styles.map((style) => {
        ...
        if (request.requiresCyclePickup && request.cyclePickupLocationId) {
            ...
            if (pickupSpot) {
                const rankedPickup = ranked.find((item) => item.spot.id === pickupSpot.id) ??
                    {
                        spot: pickupSpot,
...
function toDayKey(dayType) {
    if (dayType === "holiday")
        return "sun";
    return dayType;
}
function isSeasonalClosed(spot, month) {
...
function isWithinWeeklyHours(spot, dayType, arrival, departure) {
    if (spot.business.isAlwaysOpen)
        return true;
...
function passesLastAdmission(spot, arrival) {
...
function isSpotAvailableAt(spot, dayType, month, arrival, departure) {
    const closedDays = spot.business.operationalJudgement?.regularClosedDays ?? [];
    if (closedDays.includes(dayType))
        return false;
    if (isSeasonalClosed(spot, month))
        return false;
    if (!isWithinWeeklyHours(spot, dayType, arrival, departure))
        return false;
    if (!passesLastAdmission(spot, arrival))
        return false;
    return true;
}
function resolveEstimatedStayMinutes(spot) {
    return Math.max(15, spot.business.estimatedStayMinutesMin);
}
function sanitizeSpotsForPlan(scores) {
    const seen = new Set();
    const filtered = [];
    for (const item of scores) {
        if (seen.has(item.spot.id))
            continue;
    ...
    return [
        {
            ...
            message: params.totalMinutes <= params.allowedMinutes
                ? "滞在時間内で成立しています。"
                : "滞在時間を超過しています。",
        },
        {
...
async function buildSinglePlan(params) {
    ...
    if (chosen.length === 0) {
        throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.NO_CANDIDATE, "No scored spots for this plan style", {
            style: params.style,
        });
    }
    ...
    if (cyclePickupSpotId) {
        ...
        if (!pickupEntry) {
            throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.NO_CANDIDATE, "Cycle pickup spot is not available", {
                style: params.style,
                cyclePickupSpotId,
            });
        }
    ...
    const matrixLocations = Array.from(locationMap.entries()).map(([id, coords]) => ({
        id,
        lat: coords.lat,
        lng: coords.lng,
    }));
    ...
    let matrixDurations = {};
    try {
        const matrix = await (0, approximateRouteMatrix_1.computeApproximateRouteMatrix)({
        ...
        throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.ROUTE_UNAVAILABLE, "Failed to compute route matrix", {
            mode: matrixMode,
            detail: error instanceof Error ? error.message : String(error),
        });
    }
    const getTravelMinutes = (fromId, toId) => {
        ...
        throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.ROUTE_UNAVAILABLE, "Missing route matrix pair", {
            fromId,
            toId,
            mode: matrixMode,
        });
    ...
    for (const entry of chosen) {
        ...
        if (!isSpotAvailableAt(spot, dayType, month, arrival, departure)) {
            if (mustPickupHere) {
                throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.SPOT_CLOSED, "Cycle pickup spot is closed at departure time", {
                    spotId: spot.id,
        ...
        const returnMinutes = params.request.returnConstraint.type === "train_station" && params.returnLocation
            ? getTravelMinutes(spot.id, "__return__")
            : 0;
        if (departure + returnMinutes > endLimit) {
            if (mustPickupHere) {
                throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.TIME_OVER, "Cycle pickup spot cannot be visited within time limit", {
                    spotId: spot.id,
                    departureAt: toHhmm(departure),
    ...
    if (visitedSpotCount === 0) {
        throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.NO_CANDIDATE, "No feasible spots found");
    }
    ...
    if (params.request.returnConstraint.type === "train_station" && params.returnLocation) {
        ...
        if (arrivalAtReturn > endLimit) {
            ...
            throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.RETURN_CONSTRAINT, "Cannot satisfy train return constraint");
        }
    ...
    if (checks.some((check) => !check.passed)) {
        throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.TIME_OVER, "Constraint check failed", {
            checks,
        });
    }
    ...
    const plan = {
        ...
        title: baseTitle,
        description: params.style === "scenic"
    ...
    if (params.request.returnConstraint.type === "train_station" && params.request.origin.type === "station") {
        try {
            const trips = await (0, repository_2.getNextTrips)({
                db: params.db,
...
async function refinePlansWithAI(plans, request) {
    const refined = await callGeminiJson({
        systemInstruction: "あなたは旅行プランの編集者です。時系列や制約は変更せず、各案のタイトルと推薦理由のみを改善してください。JSONのみ返してください。",
    ...
    return plans.map((plan) => {
        ...
        if (!item) {
            throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.AI_PARSE_FAILED, "AI refinement result missing plan id", {
                planId: plan.id,
            });
        }
        return {
...
async function resolveReturnLocation(request, origin, db) {
    if (request.returnConstraint.type !== "train_station")
        return null;
    const node = await (0, repository_2.getNode)({ db, nodeId: request.returnConstraint.stationId });
    if (!node) {
        throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.RETURN_CONSTRAINT, "Return station node is missing", {
...
async function resolveOriginLocation(request, db) {
    if (request.origin.type === "current_location") {
        return { lat: request.origin.lat, lng: request.origin.lng };
    }
    const stationNode = await (0, repository_2.getNode)({ db, nodeId: request.origin.id });
    if (!stationNode) {
        throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.ROUTE_UNAVAILABLE, "Origin station node is missing", {
            originId: request.origin.id,
        });
    }
    return { lat: stationNode.location.lat, lng: stationNode.location.lng };
}
async function generatePlans(params) {
    const db = params.db ?? (0, firebase_admin_1.getFirebaseAdminDb)();
    const doc = await (0, repository_1.getPlanRequestById)({ db, planRequestId: params.planRequestId });
    if (!doc) {
        throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.INTERNAL, "Plan request not found");
    }
    const request = doc.normalizedRequest;
    await (0, repository_1.updatePlanRequestStatus)({
        db,
    ...
    await (0, repository_1.savePlanRequestIntent)({ db, planRequestId: params.planRequestId, intent });
    await (0, repository_1.updatePlanRequestStatus)({
        db,
        planRequestId: params.planRequestId,
        status: constants_1.PLAN_REQUEST_STATUS.GENERATING,
        generationStage: constants_1.PLAN_GENERATION_STAGE.CANDIDATE_SELECTION,
        progressPercent: 28,
        traceMessage: "Selecting candidate spots",
    });
    const publishedSpots = await (0, spotService_1.listSpots)({ db, filters: { status: "published" } });
    if (publishedSpots.length === 0) {
        throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.NO_CANDIDATE, "No published spots available");
    }
    ...
    await (0, repository_1.updatePlanRequestStatus)({
        ...
        planRequestId: params.planRequestId,
        status: constants_1.PLAN_REQUEST_STATUS.GENERATING,
        generationStage: constants_1.PLAN_GENERATION_STAGE.ROUTE_PLANNING,
        progressPercent: 56,
    ...
    for (const meta of metas) {
        let built = null;
        let attempts = 0;
        while (!built && attempts < constants_1.PLAN_GENERATION_MAX_ATTEMPTS) {
            attempts += 1;
            try {
                ...
                if (error instanceof PlanGenerationFailure) {
                    ...
                    if (attempts >= constants_1.PLAN_GENERATION_MAX_ATTEMPTS) {
    ...
    if (plans.length < 3) {
        if (terminalFailures.length > 0) {
            const byPriority = [
                constants_1.PLAN_GENERATION_ERROR_CODE.ROUTE_UNAVAILABLE,
                constants_1.PLAN_GENERATION_ERROR_CODE.AI_PARSE_FAILED,
                constants_1.PLAN_GENERATION_ERROR_CODE.RETURN_CONSTRAINT,
                constants_1.PLAN_GENERATION_ERROR_CODE.TIME_OVER,
                constants_1.PLAN_GENERATION_ERROR_CODE.SPOT_CLOSED,
                constants_1.PLAN_GENERATION_ERROR_CODE.NO_CANDIDATE,
            ];
        ...
        throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.NO_CANDIDATE, "Unable to produce three valid plans", {
    ...
    await (0, repository_1.updatePlanRequestStatus)({
        ...
        planRequestId: params.planRequestId,
        status: constants_1.PLAN_REQUEST_STATUS.GENERATING,
        generationStage: constants_1.PLAN_GENERATION_STAGE.AI_REFINEMENT,
        progressPercent: 76,
    ...
    await (0, repository_1.updatePlanRequestStatus)({
        ...
        planRequestId: params.planRequestId,
        status: constants_1.PLAN_REQUEST_STATUS.GENERATING,
        generationStage: constants_1.PLAN_GENERATION_STAGE.VALIDATING,
        progressPercent: 92,
    ...
    for (const plan of refinedPlans) {
        ...
        if (hasViolation) {
            throw new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.TIME_OVER, "Constraint violation detected in final plans");
        }
    }
    const result = {
        plans: refinedPlans,
    ...
    await (0, repository_1.savePlanRequestResult)({
        db,
        planRequestId: params.planRequestId,
        result,
    });
}
async function runPlanGenerationWithFailureHandling(params) {
    const db = params.db ?? (0, firebase_admin_1.getFirebaseAdminDb)();
    try {
        await generatePlans(params);
    }
    catch (error) {
        const normalizedError = error instanceof PlanGenerationFailure
            ? error
            : new PlanGenerationFailure(constants_1.PLAN_GENERATION_ERROR_CODE.INTERNAL, "Unexpected generation failure", {
        ...
        await (0, repository_1.savePlanRequestError)({
            db,
            planRequestId: params.planRequestId,
            error: {
                code: normalizedError.code,
