...
import type { SpotPrimaryCategory, SpotTransport } from "../spots/spotTypes";

type CsvRow = {
  id?: unknown;
  name?: unknown;
  type?: unknown;
  isAlwaysOpen?: unknown;
  openingHoursText?: unknown;
  weeklyHours?: unknown;
  notes?: unknown;
  nearestStations?: unknown;
  supportedTransports?: unknown;
  estimatedStayMinutesMin?: unknown;
  estimatedStayMinutesMax?: unknown;
  themes?: unknown;
  "weatherSuitability.rainy"?: unknown;
  rainFallbackCandidate?: unknown;
...
const TYPE_TO_CATEGORY: Record<string, SpotPrimaryCategory> = {
  飲食: "eat",
  宿泊: "stay",
  景観: "see",
  駅: "see",
...
const TRANSPORT_TO_VALUE: Record<string, SpotTransport> = {
  徒歩: "walk",
  自転車: "rental_cycle",
  車: "car",
  バス: "bus",
...
const RAINY_TO_VALUE: Record<string, "good" | "ok" | "bad"> = {
  "◎": "good",
  "○": "good",
  "△": "ok",
  "×": "bad",
...
function splitCsvList(value: unknown): string[] {
...
function parseRows(csvPath: string): CsvRow[] {
  const workbook = readFile(csvPath, { raw: false, codepage: 65001 });
  ...
  return utils.sheet_to_json<CsvRow>(sheet, { defval: "" });
}
...
async function main() {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    const supportedTransports = dedupe(
      splitCsvList(row.supportedTransports)
        .map((label) => TRANSPORT_TO_VALUE[label])
        .filter((value): value is SpotTransport => Boolean(value)),
    );

    const nearestStations = dedupe(splitCsvList(row.nearestStations))
      .map((stationName) => STATION_NAME_TO_INFO[stationName as keyof typeof STATION_NAME_TO_INFO])
      .filter((info): info is (typeof STATION_NAME_TO_INFO)[keyof typeof STATION_NAME_TO_INFO] => Boolean(info))
      .map((station) => {
        const distanceMeters =
          station.stationId === spotId
            ? 0
            : Math.round(
                haversineMeters(
    ...
    const resolvedOpeningHoursText =
      openingHoursText ?? (isAlwaysOpen === true ? existing.business.openingHoursText ?? "終日開放" : null);
    const mergedRegularHolidaysText = [weeklyHours, notes]
      .filter((value): value is string => typeof value === "string" && value.trim().length > 0)
      .join(" / ");
    const resolvedRegularHolidaysText =
      mergedRegularHolidaysText.length > 0 ? mergedRegularHolidaysText : existing.business.regularHolidaysText;
  ...
  console.log(
    `[importSpotsOpenInfoDefaultsCsv] completed file=${csvPath} totalRows=${rows.length} updated=${updatedCount} skipped=${skippedCount}`,
  );
  if (skipped.length > 0) {
