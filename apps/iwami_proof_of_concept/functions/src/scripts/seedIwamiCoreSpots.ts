...
import { createSpot, getSpotById, updateSpot } from "../spots/spotService";
import type { SpotPrimaryCategory, SpotStationAreaType } from "../spots/spotTypes";
...
type StationRef = {
  stationId: "iwami-station" | "higashihama-station" | "oiwa-station";
  stationName: string;
  lat: number;
  lng: number;
};
...
function inferAreaName(stationAreaType: SpotStationAreaType): string | null {
  if (stationAreaType === "iwami_station_area") return "岩美駅周辺";
  if (stationAreaType === "higashihama_station_area") return "東浜駅周辺";
  if (stationAreaType === "oiwa_station_area") return "大岩駅周辺";
  return "岩美町";
}

function buildShortName(nameJa: string): string {
  const noParen = nameJa.replace(/（[^）]*）/g, "").trim();
  return noParen.length > 0 ? noParen : nameJa;
}

function resolveNearestStation(seed: SpotSeed) {
...
function categoryDefaults(category: SpotPrimaryCategory) {
  if (category === "eat") {
    return {
  ...
  if (category === "shop") {
    return {
      ...
      priceType: "purchase_optional" as const,
      indoorOutdoor: "indoor" as const,
      foodScore: 2,
      shoppingScore: 5,
      experienceScore: 2,
...
function buildCreateInput(seed: SpotSeed) {
  ...
  const supportedTransports =
    seed.slug === "iwami-station" || seed.slug === "higashihama-station" || seed.slug === "oiwa-station"
      ? (["walk", "car", "bus", "train"] as const)
      : (["walk", "car", "bus"] as const);
  ...
  return {
    ...
    plannerAttributes: {
      ...
      timeOfDaySuitability: ["morning", "daytime", "sunset"] as const,
      visitPace: ["normal_stop"] as const,
      withWho: ["solo", "friends", "couple", "family"] as const,
...
async function main() {
  ...
  for (const [index, seed] of SPOTS.entries()) {
    const existing = await getSpotById({ db, spotId: seed.slug });
    const shortName = buildShortName(seed.nameJa);
  ...
  console.log(
    `[seedIwamiCoreSpots] completed total=${SPOTS.length} created=${createdCount} updated=${updatedCount}`,
  );
}
