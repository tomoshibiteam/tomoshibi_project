...
type SpotWeekDay = (typeof ALL_WEEK_DAYS)[number];
const DAY_ORDER: SpotBusinessDay[] = ["mon", "tue", "wed", "thu", "fri", "sat", "sun", "holiday"];
const DEFAULT_OPERATIONAL_JUDGEMENT: SpotOperationalJudgement = {
  regularClosedDays: [],
...
function parseDayToken(rawToken: string): SpotBusinessDay | null {
  const token = rawToken.toLowerCase().replace(/[^a-z\u3040-\u30ff\u4e00-\u9fff]/g, "");
  if (!token) return null;

  if (token.includes("holiday") || token.includes("祝")) return "holiday";
  if (token.startsWith("mon") || token.includes("月")) return "mon";
  if (token.startsWith("tue") || token.includes("火")) return "tue";
  if (token.startsWith("wed") || token.includes("水")) return "wed";
  if (token.startsWith("thu") || token.includes("木")) return "thu";
  if (token.startsWith("fri") || token.includes("金")) return "fri";
  if (token.startsWith("sat") || token.includes("土")) return "sat";
  if (token.startsWith("sun") || token.includes("日")) return "sun";
  return null;
}

function parseDayRange(fromDay: SpotWeekDay, toDay: SpotWeekDay): SpotWeekDay[] {
...
function parseDaysFromExpression(rawExpression: string): SpotBusinessDay[] {
  ...
  for (const part of parts) {
    ...
    if (rangeMatch) {
      ...
      if (startDay === "holiday" || endDay === "holiday") continue;
      for (const day of parseDayRange(startDay, endDay)) days.add(day);
      continue;
    }

    const tokenDay = parseDayToken(part);
    if (tokenDay) days.add(tokenDay);
  }

  return [...days];
}

function dedupeDays(days: SpotBusinessDay[]): SpotBusinessDay[] {
  const seen = new Set<SpotBusinessDay>();
  const out: SpotBusinessDay[] = [];
...
function extractClosedDays(text: string): SpotBusinessDay[] {
  const out: SpotBusinessDay[] = [];

  const englishMatches = text.matchAll(
    /([A-Za-z\s\/,\-〜～]+?)\s*(?:regular\s*)?closed(?:\b|[^a-z])/gi,
  );
  ...
  const japaneseMatches = text.matchAll(
    /([月火水木金土日](?:曜(?:日)?)?(?:[\/、，・\-〜～]?[月火水木金土日](?:曜(?:日)?)?)*)\s*(?:定休|休館|休業|休み|休)/g,
  );
...
function parseTimeRanges(rawText: string): SpotWeeklyHoursRange[] {
...
function parseWeeklyHoursFromNarrative(input: {
  ...
  weeklyText: string;
  openingText: string;
  closedDays: SpotBusinessDay[];
}): SpotWeeklyHours | undefined {
  if (input.isAlwaysOpen) return undefined;

  const weeklyHours: SpotWeeklyHours = {};
  ...
  if (
    /(チェックイン|チェックアウト|運行時間帯|準拠|予約制|通行止め|立入不可|カレンダー制|随時対応)/.test(
      input.openingText,
    )
  ) {
    return undefined;
  ...
  if (openingRanges.length === 0) return undefined;
  ...
  const primaryRange = openingRanges.reduce((best, current) => {
    ...
    const currentDuration =
      Number(current.close.slice(0, 2)) * 60 +
      Number(current.close.slice(3)) -
      (Number(current.open.slice(0, 2)) * 60 + Number(current.open.slice(3)));
    return currentDuration > bestDuration ? current : best;
  });

  const closedSet = new Set<SpotWeekDay>(input.closedDays.filter((day): day is SpotWeekDay => day !== "holiday"));
...
function extractSeasonalClosures(text: string): SpotSeasonalClosure[] {
  ...
  for (const sentence of sentences) {
    if (!/(休業|休館|休止|利用不可|運休|冬季|夏季|年末年始|GW)/.test(sentence)) continue;

    const fixedRangeMatches = sentence.matchAll(
...
function deriveLastAdmission(input: {
  explicitLastEntryTime: string | null;
  openingText: string;
  regularHolidayText: string;
}): SpotOperationalJudgement["lastAdmission"] {
  ...
  if (!mergedText) return { ...DEFAULT_OPERATIONAL_JUDGEMENT.lastAdmission };
  ...
  const timeMatch =
    mergedText.match(/(?:L\.?\s*O\.?|ラストオーダー)\s*[:：]?\s*([0-2]?\d[:：][0-5]\d)/i) ??
  ...
  if (!parsedTime) return { ...DEFAULT_OPERATIONAL_JUDGEMENT.lastAdmission };
...
function hasManualReviewKeywords(text: string): boolean {
  return /(要確認|不定休|臨時|変動|カレンダー制|通行止め|立入不可|貸切|別管理|可能性|相談可|予約制|一時中断|休止)/.test(
    text,
  );
}

function mergeOperationalJudgement(
  base: SpotOperationalJudgement,
  override: SpotOperationalJudgement | undefined,
): SpotOperationalJudgement {
...
export function deriveBusinessOperationalData(
  input: DeriveBusinessOperationalDataInput,
): DeriveBusinessOperationalDataResult {
  ...
  const needsManualReview =
    hasManualReviewKeywords(mergedText) || (!input.isAlwaysOpen && !hasStructuredHours && openingText.length > 0);

  const generated: SpotOperationalJudgement = {
    regularClosedDays,
