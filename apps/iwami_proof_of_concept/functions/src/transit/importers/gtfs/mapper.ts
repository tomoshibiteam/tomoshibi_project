import { TRANSIT_TIMEZONE } from "../../constants";
import {
...
const WEEKDAY_DAY_TYPES: TransitDayType[] = ["mon", "tue", "wed", "thu", "fri"];
const WEEKEND_HOLIDAY_DAY_TYPES: TransitDayType[] = ["sat", "sun", "holiday"];
...
function toBusStopNodeId(stopId: string): string {
  return `gtfs_bus_stop_${sanitizeIdentifier(stopId)}`;
}

function resolveLinkedSpotId(stopNameJa: string): string | null {
  const normalized = normalizeForMatching(stopNameJa);
  if (normalized.includes("岩美駅")) return "iwami-station";
  if (normalized.includes("道の駅きなんせ岩美")) return "kinanse-iwami-roadside-station";
  if (normalized.includes("岩美町観光協会")) return "iwami-tourism-association";
  return null;
}

function isStationHubCandidate(stopNameJa: string): boolean {
  const normalized = normalizeForMatching(stopNameJa);
  return (
    normalized.includes("岩美駅") ||
...
function parseGtfsTimeToMinutes(value: string): number {
...
function isWeekend(dateYYYYMMDD: string): boolean {
...
function resolveCalendarBucketsByService(params: {
  ...
  calendarDates: ParsedGtfsData["calendarDates"];
}): {
  bucketsByServiceId: Map<string, Set<"weekday" | "weekend_holiday">>;
  serviceIdsByBucket: {
    weekday: Set<string>;
    weekend_holiday: Set<string>;
  };
} {
  const bucketsByServiceId = new Map<string, Set<"weekday" | "weekend_holiday">>();

  for (const calendar of params.calendars) {
    const buckets = new Set<"weekday" | "weekend_holiday">();
    if (
      isOne(calendar.monday) ||
  ...
  for (const calendarDate of params.calendarDates) {
    if (calendarDate.exception_type !== "1") continue;

    const bucket = isWeekend(calendarDate.date) ? "weekend_holiday" : "weekday";
    const existing = bucketsByServiceId.get(calendarDate.service_id) ?? new Set<"weekday" | "weekend_holiday">();
    existing.add(bucket);
  ...
  const serviceIdsByBucket = {
    ...
    weekend_holiday: new Set<string>(),
  };

  for (const [serviceId, buckets] of bucketsByServiceId.entries()) {
    if (buckets.has("weekday")) serviceIdsByBucket.weekday.add(serviceId);
    if (buckets.has("weekend_holiday")) serviceIdsByBucket.weekend_holiday.add(serviceId);
  }

  return { bucketsByServiceId, serviceIdsByBucket };
}

function resolveBaseService(params: { route: GtfsRouteRow }): {
  serviceId: string;
  lineName: string;
...
function buildTransportNodes(params: {
  ...
  return params.stops
    .map((stop) => {
      ...
      return transportNodeRecordSchema.parse({
        nodeId,
        nodeType: "bus_stop",
        nameJa,
        location: {
...
export function mapGtfsToTransit(params: {
  parsed: ParsedGtfsData;
  options: GtfsImportOptions;
}): MappedTransitData {
  const status = params.options.status ?? DEFAULT_STATUS;
  const timezone = params.options.timezone ?? TRANSIT_TIMEZONE;
  ...
  const serviceMap = new Map<
    string,
    {
      ...
      gtfsRouteIds: Set<string>;
      gtfsDirectionIds: Set<string>;
      gtfsServiceIds: Set<string>;
    }
  >();
  ...
  for (const trip of params.parsed.trips) {
    ...
    if (!existing) {
      const record = transitServiceRecordSchema.parse({
    ...
    if (trip.direction_id) existing.gtfsDirectionIds.add(trip.direction_id);
    existing.gtfsServiceIds.add(trip.service_id);
  }

  const transitServices = Array.from(serviceMap.values())
    .map((entry) => {
      const merged = transitServiceRecordSchema.parse({
        ...entry.record,
        metadata: {
  ...
  const transitCalendars: TransitCalendarRecord[] = [
  ...
  const edgeMap = new Map<string, TransitEdgeScheduleRecord>();
  const tripContextsByGtfsTripId: Record<string, GtfsTripContext> = {};
  ...
  for (const [tripId, stopTimes] of stopTimesByTripId.entries()) {
    ...
    const resolved = resolveBaseService({ route });
    const buckets = bucketsByServiceId.get(trip.service_id) ?? new Set<"weekday" | "weekend_holiday">(["weekday"]);
    ...
    .map((schedule) => {
      ...
      const uniqueTrips = schedule.trips
        .sort((a, b) => {
          ...
          if (a.arriveMinutes !== b.arriveMinutes) return a.arriveMinutes - b.arriveMinutes;
